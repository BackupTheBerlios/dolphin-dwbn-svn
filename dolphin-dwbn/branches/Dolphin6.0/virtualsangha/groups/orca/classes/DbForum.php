...
class DbForum extends BxDb
{
	...
	function searchMessages ($s, $u, $f, $type, $posts, $limit)
	{
		...
		{
			return $this->getAll ("
	SELECT DISTINCTROW t4.`cat_id`, `cat_name`, t3.`forum_id`, `forum_title`, t2.`topic_id`, `topic_title`, `post_id`, DATE_FORMAT(FROM_UNIXTIME(t1.`when`),'{$gConf['date_format']}') AS `date`, `user` $x
	...
	LIMIT $limit");
		...
		{
			return $this->getAll ("
	SELECT t4.`cat_id`, `cat_name`, t3.`forum_id`, `forum_title`, t2.`topic_id`, `topic_title`, DATE_FORMAT(FROM_UNIXTIME(`first_post_when`),'{$gConf['date_format']}') AS `date`, `first_post_user` AS `user`
	...
	function getCategs ()
	{
		return $this->getAll ("SELECT `cat_id`, `cat_name`, `cat_icon` FROM " . TF_FORUM_CAT . " ORDER BY `cat_order` ASC");
	}
	...
	{
		global $gConf;
		return $this->getAll ( "SELECT `forum_id`, `cat_id`, `forum_title`, `forum_desc`, `forum_type`, `forum_posts`, `forum_topics`, DATE_FORMAT(FROM_UNIXTIME(`forum_last`),'{$gConf['date_format']}') AS `forum_last`, `forum_last` AS `forum_last_ts` FROM " . TF_FORUM . ($c ? " WHERE `cat_id` = '$c'": '') . ' ORDER BY `forum_id` ASC');
	}

	function getForum ($f)
	{
		global $gConf;
		return $this->getRow ( "SELECT `cat_id`, `forum_title`, `forum_desc`, `forum_type`, `forum_posts`, DATE_FORMAT(FROM_UNIXTIME(`forum_last`),'{$gConf['date_format']}') AS `forum_last` FROM " . TF_FORUM . " WHERE `forum_id` = '$f' LIMIT 1");
	}

	function getForumIdByTopicId ($topic_id)
	{
		return $this->getOne ( "SELECT `forum_id` FROM" . TF_FORUM_TOPIC . " WHERE `topic_id` = '$topic_id' LIMIT 1");
	}
	
	function getForumTypeByTopicId ($topic_id)
	{
		return $this->getOne ( "SELECT `forum_type` FROM " . TF_FORUM . " INNER JOIN " . TF_FORUM_TOPIC . " USING(`forum_id`) WHERE `topic_id` = '$topic_id' LIMIT 1");
	}

	function getForumTypeByPostId ($post_id)
	{
		return $this->getOne ( "SELECT `forum_type` FROM " . TF_FORUM . " INNER JOIN " . TF_FORUM_POST . " USING(`forum_id`) WHERE `post_id` = '$post_id' LIMIT 1");
	}

	function getForumIdByPostId ($post_id)
	{
		return $this->getOne ( "SELECT `forum_id` FROM " . TF_FORUM_POST . " WHERE `post_id` = '$post_id' LIMIT 1");
	}
	...
	function getMyFlaggedTopics ($u)
	{
		...
		$sql = "SELECT f1.`topic_id`, `topic_title`, `last_post_when`, `topic_posts` AS `count_posts` FROM " . TF_FORUM_TOPIC . " AS f1 INNER JOIN "  . TF_FORUM_FLAG . " AS f2 USING (`topic_id`) WHERE f2.`user` = '$u' ORDER BY `last_post_when` DESC";
	...
	function getMyThreadsTopics ($u)
	{
		...
		$sql = "SELECT DISTINCTROW f1.`topic_id`, `topic_title`, `last_post_when`, `topic_posts` AS `count_posts` FROM " . TF_FORUM_TOPIC . " AS f1 INNER JOIN "  . TF_FORUM_POST . " AS f2 USING (`topic_id`) WHERE f2.`user` = '$u' ORDER BY `last_post_when` DESC";
	...
	function getTopic ($t)
	{
		return $this->getRow ( "SELECT `topic_id`, `topic_title`, `forum_title`, `forum_desc`, `forum_type`, f1.`forum_id`, `cat_id`, `topic_locked` FROM " . TF_FORUM_TOPIC . " AS f1 INNER JOIN " . TF_FORUM . " USING (`forum_id`) WHERE f1.`topic_id` = '$t' LIMIT 1");
	}
	...
	function newTopic ($f, $title, $text, $sticky, $user)
	{		
		$ts = time ();
		...
		// add topic title
		if (!$this->query ("INSERT INTO" . TF_FORUM_TOPIC . " SET `topic_posts` = 1, `forum_id` = '$f', `topic_title` = '$title', `when` = '$ts', `first_post_user` = '$user', `first_post_when` = '$ts', `last_post_user` = '$user', `last_post_when` = '$ts', `topic_sticky` = '$sticky'"))
			return false;
	...
	function getPosts ($t, $u)
	{
		...
		$sql =  "SELECT `forum_id`, `topic_id`, t1.`post_id`, `user`, `post_text`, `votes`, $sql_add1 DATE_FORMAT(FROM_UNIXTIME(t1.`when`),'{$gConf['date_format']}') AS `when` FROM " . TF_FORUM_POST . " AS t1 $sql_add2 WHERE `topic_id` = '$t' ORDER BY t1.`when` ASC";
	...
	function getUserPostsList ($user, $sort, $limit = 10)
	{
		...
		$sql =  "
		SELECT t1.`forum_id`, t1.`topic_id`, t2.`topic_title`, t1.`post_id`, t1.`user`, LEFT(`post_text`, 256) AS `post_text`, DATE_FORMAT(FROM_UNIXTIME(t1.`when`),'{$gConf['date_format']}') AS `when`
			FROM " . TF_FORUM_POST . " AS t1
	...
	{
		$sql = "SELECT `topic_id` FROM " . TF_FORUM_POST . " WHERE `post_id` = '$post_id'";
	...
	function getLivePosts ($c, $ts)
	{
		...
		$sql = "SELECT (UNIX_TIMESTAMP() - tp.`when`) AS `sec`, tp.`when` AS `ts`, `tp`.`user`, `tp`.`post_id`, LEFT(`tp`.`post_text`, {$gConf['live_tracker_desc_len']}) AS `post_text`, tt.`topic_id`, `tt`.`topic_title`, tf.`forum_id`, `tf`.`forum_title`, tc.`cat_id`, `tc`.`cat_name` FROM " . TF_FORUM_POST . " AS tp INNER JOIN " . TF_FORUM_TOPIC . " AS tt USING(`topic_id`) INNER JOIN " . TF_FORUM . " AS tf ON (tf.`forum_id` = tp.`forum_id` AND `tt`.`forum_id` = tf.`forum_id`) INNER JOIN " . TF_FORUM_CAT . " AS tc USING(`cat_id`) WHERE $where ORDER BY tp.`when` $order LIMIT $c";
