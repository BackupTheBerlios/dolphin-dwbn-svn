...
function getDiscountedAmount( $number, $amount )
{
	$number = (int)$number;
	$total = 0;
	$arr = db_arr("SELECT `DiscountPercent` FROM `ContactDiscounts` WHERE `ProfilesNumber` <= {$number} ORDER BY `ProfilesNumber` DESC LIMIT 1");
	if ( !$arr )
	{
		return $amount;
	}
	...
	{
		return $arr['DiscountPercent'] / 100.0 * $amount;  // divide by 100 as discont in percents
	}
}
...
//	db_res( "DELETE FROM `ProfilesRelations` WHERE `sourceID` = '{$ID}' OR `targetID` = '{$ID}';" );
	db_res( "DELETE FROM `Messages` WHERE Recipient = {$ID} " );
	db_res( "DELETE FROM `Guestbook` WHERE Recipient = {$ID} " );
	db_res( "DELETE FROM `MemCredits` WHERE `ID` = {$ID}" );
		...
		case 'G':
			$onl_arr = db_arr( "SELECT	COUNT(DISTINCT IDMember) AS count_onl
								...
								WHERE
									...
									(LastNavTime > SUBDATE(NOW(), INTERVAL $min MINUTE))" );
			break;
		// from country
		case 'c':
		case 'C':
