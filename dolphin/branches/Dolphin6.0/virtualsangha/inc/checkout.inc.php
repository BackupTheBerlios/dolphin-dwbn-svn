 ...
 * 			  'credits_amount',			- sum of credit purchase transactions
...
function getFinanceAffStat( $affID, $tranArray )
{
	...
	$arr = db_arr( "SELECT SUM( `Amount` ) FROM `Transactions` INNER JOIN `aff_members` ON (`idProfile` = `IDMember`) WHERE {$tranFilter} AND `idAff` = {$affID} AND `Status` = 'approved' AND `Data` LIKE '%{$separator}credits{$separator}%' AND `Currency` = '{$currency_code}'" );
	$fin['credits_amount'] = sprintf( "%.2f", $percent * (float)$arr[0] );
...
function getFinanceStat( $tranArray )
{
	...
	$arr = db_arr( "SELECT SUM( `Amount` ) FROM `Transactions` WHERE {$tranFilter} AND `Status` = 'approved' AND `Data` LIKE '%{$separator}credits{$separator}%' AND `Currency` = '{$currency_code}'" );
	$fin['credits_amount'] = sprintf( "%.2f", (float)$arr[0] );
