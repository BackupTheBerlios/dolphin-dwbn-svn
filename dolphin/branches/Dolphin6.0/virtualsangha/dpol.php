...
echo "<?xml version='1.0' encoding='$langHTMLCharset'?>";
...
// VOTE action --------------------------------------------------------
	...
	if (  '' != $_GET['ID'] )
	{
	    ...
	    if ( '' != $_GET['param'] )
	    {
			...
			$query = "UPDATE ProfilesPolls SET poll_results = '$results', poll_total_votes = '$poll_total_votes' WHERE id_poll='" . (int)$_GET['ID'] . "'";
			db_res( $query );
	    }
