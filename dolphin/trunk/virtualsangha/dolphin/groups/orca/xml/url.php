 ...
 *******************************************************************************/
    ...
    case 'user_perm':        
        if ('url' == $gConf['integration'])
        {
            $integration_file = $gConf['url']['integration'] . "?action=$action&forum_id=$forum_id&user=" . urlencode($user) . $add_to_url;
        }
        else
        {
            $integration_file['file'] = $gConf['dir']['integration'];
            $integration_file['vars'] = array_merge (array ('action' => $action, 'user' => $user, 'forum_id' => $forum_id), $add_to_array);
        }
        break;        
    default:        
        if ('url' == $gConf['integration'])
