(function($) {
	...
	$.extend($.ui.slider.prototype, {
		...
		destroy: function() {
			...
			if( this.multipleHandles ) {
				for( var iInd = 0; iInd < this.interactions.length; iInd ++ )
					this.interactions[iInd].destroy();
			} else {
		...
		moveTo: function(op) {
			var value = op[0] || 0;
			var index = op[1] || 0;
			var fireTriggers = op[2] || false;
			var myHandle;

			if(this.multipleHandles) {
				if( index == undefined )
					index = 0;
				else if( this.interactions[index] == undefined )
					return false;
				
				myHandle = this.interactions[index];
			} else if( index == 0 )
				myHandle = this.interaction;
			else
				return false;
			
			var o = myHandle.options;
			var offset = $(myHandle.element).offsetParent().offset({ border: false });
			myHandle.pickValue = myHandle.curValue;
			...
			myHandle.curValue = (Math.round((m/p)*o.realValue));
			if(o.stepping) {
				myHandle.curValue = Math.round(myHandle.curValue/o.stepping)*o.stepping;
				m = ((myHandle.curValue)/o.realValue) * p;
			}

			$(myHandle.element).css(prop, m);
			
			/* if(fireTriggers && myHandle.pickValue != myHandle.curValue && !p)
				$(this.element).triggerHandler("slidechange", [this.element, this.prepareCallbackObj(myHandle)], o.change);
			
			if(fireTriggers)
				$(this.element).triggerHandler("slide", [this.element, this.prepareCallbackObj(myHandle)], o.slide); */
		}
	});
