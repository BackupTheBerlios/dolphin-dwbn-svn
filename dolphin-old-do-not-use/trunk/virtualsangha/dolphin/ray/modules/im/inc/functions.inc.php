...
function refreshIMUsers($iSndId, $iRspId)
{
	...
	//--- delete idle users ---//
	getResult("DELETE FROM `" . MODULE_DB_PREFIX . "Contacts` WHERE `When`<=" . ($iCurrentTime - $iIdleTime));
	//--- delete old messages ---//
	getResult("DELETE FROM `" . MODULE_DB_PREFIX . "Messages` WHERE `When`<=" . ($iCurrentTime - $iIdleTime));
}
