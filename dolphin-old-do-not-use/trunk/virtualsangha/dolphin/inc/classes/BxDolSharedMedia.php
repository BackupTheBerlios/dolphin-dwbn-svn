	...
	*/
	...
	function showActionList($aFile) {
		if ($this->iViewer) {
			...
			$sOnClickTempl = "javascript: window.open( '{$this->aConfigSite['url']}{$this->sType}Actions.php?{action}', 'photo', 'width=500, height=380, menubar=no,status=no,resizable=yes,scrollbars=yes,toolbar=no,location=no' );";
			...
			foreach ($aActions as $sKey => $aVal) {
				...
				$sOnClick = str_replace('__fileID__',  '&amp;fileID='.$aFile['medID'], $sOnClick);
