...
<div>
	<div class="disignBoxFirst">
		<div class="boxFirstHeader">{$sBreadCrumb}</a></div>
		<div class="boxContent">
			{$sPostString}
		</div>
	</div>
...
EOF;
	...
	/*function ActionEditComment(){
		...
		$sCheckSQL = "SELECT `SenderID`,`PostID` FROM `BlogPostComments` WHERE `cmt_id`={$iCommId}";
		...
		if (( $this->aBlogConf['visitorID']==$iPostOwnerID || $this->aBlogConf['visitorID']==$iSenderID || $this->bAdminMode==true) && $iCommId > 0) {
			...
			$query = "UPDATE `BlogPostComments` SET `CommentText` = '{$sMessage}' WHERE `BlogPostComments`.`cmt_id` = {$iCommId} LIMIT 1 ;";
			$sqlRes = db_res( $query );
	...
	/*function ActionDeleteComment(){
		...
		$PostID = db_arr($sCheckPostSQL);
		$iPostOwnerID = $PostID['OwnerID'];
		if (( $this->aBlogConf['visitorID'] == $iPostOwnerID || $this->aBlogConf['visitorID'] == $iSenderID || $this->bAdminMode) && $iCommId > 0) {
    ...
    function genBlogFormUrl() {
		$sMainLink = ($_SERVER['PHP_SELF'] != 'post_mod_blog.php') ? 'blogs.php' : 'post_mod_blog.php';
