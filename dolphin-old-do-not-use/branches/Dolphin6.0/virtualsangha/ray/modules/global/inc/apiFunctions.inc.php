 ...
 */
function printFiles($sModule, $sFolder = "langs", $bGetDate = false, $bGetLangNames = false)
{
	global $sIncPath;
	global $sModulesUrl;
	...
	for($i=0; $i<count($aEnabledFiles); $i++)
		if(isset($aRealFiles[$aEnabledFiles[$i]]))
		{
			...
			{
				if($bGetLangNames && $sFolder == "langs")
				{
					$sLangName = getSettingValue($sModule, "_name_", $sFile, false, $sFolder = "langs");
					if(empty($sLangName)) $sLangName = $sFile;
					$sContents .= parseXml($aXmlTemplates['file'], $sFile, $sLangName, "");
				}
