...
?>
		...
		<tr>
			<td class="section_row" style="text-align: left; vertical-align: top;">
				...
				<b>Place:</b> <?= _t('__'.$prof['countries'][$eventArr['Country']]) .', '. process_line_output($eventArr['City']) .', '. process_line_output($eventArr['Place']) ?><br />
...
<?
	...
	{
		echo "
							<option value=\"{$key}\" ". ( $selectedCountry == $key ? 'selected="selected"' : '' ) ." >". _t('__'.$value) ."</option>";
	}
?>
						</select>
					</td>
		...
		if ( $event_id )
		{
			...
			if ( $_POST['send_message'] == 'Send message' )
			{
				...
				foreach ( $_POST as $key => $value )
				{
					if ( (int)$key && $value == 'on' )
					{
						$part_arr = db_arr( "SELECT `Profiles`.`ID`, `Profiles`.`NickName`, `Profiles`.`Email`, `Profiles`.`EmailFlag`, `SDatingParticipants`.`ParticipantUID`, `SDatingEvents`.`Title`, `SDatingEvents`.`Place`, `SDatingEvents`.`EventStart` FROM `SDatingParticipants`
...
<?
	...
	{
		echo "
							<option value=\"{$key}\" ". ( $sel_country == $key ? 'selected="selected"' : '') ." >". _t('__'.$value) ."</option>";
	}
?>
						</select>
					</td>
