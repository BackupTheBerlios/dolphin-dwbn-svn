...
$_page_cont[$_ni]['credits'] =  ( $en_credits ? PageCompCredits() : '' );
...
function PageCompStatus( $memberID )
{
	...
	$ret .= GetMembershipStatus($memberID, false, $en_credits);
...
<div class="credits_box">
	<form id="buyCreditsForm" action="<?= $site['url'] . 'checkout.php' ?>" method="post" style="margin: 2px">
	<input type="hidden" name="action" value="calculate" />
	<input type="hidden" name="checkout_action" value="credits" />
	<input type="hidden" name="allow_subscribe" value="on" />
	<table cellpadding="4" cellspacing="0" border="0" width="100%">
		<tr>
			<td align="left">
				<select class="no" name="data" style="vertical-align: middle;">
<?
	$maxIndex = ($maxCredits - $minCredits) / $creditStep;
	for ( $credIndex = 0; $credIndex <= $maxIndex; $credIndex++ )
	{
		$creditsAmount = $minCredits + $credIndex * $creditStep;
		$creditsPrice = $creditsAmount / $credit2money - $credIndex * $creditDiscountStep;
		echo "<option value=\"{$credIndex}\">{$creditsAmount} ". _t('_credits') ." - {$doll}{$creditsPrice}</option>\n";
	}
?>
				</select>
			</td>
			...
			<td align="right" width="60">
				<a href="javascript:void(0);" onClick=" javascript:window.open('explanation.php?explain=credits', '', 'width=<?= $oTemplConfig -> popUpWindowWidth ?>,height=<?= $oTemplConfig -> popUpWindowHeight ?>,menubar=no,status=no,resizable=no,scrollbars=yes,toolbar=no, location=no' );"><?= _t("_Explanation") ?></a>
			</td>
		</tr>
	</table>
	</form>
...
<?
	...
	return DesignBoxContent( _t("_Membership Credits"), $ret, $oTemplConfig -> PageCompCredits_db_num );
}
