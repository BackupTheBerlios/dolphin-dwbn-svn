...
if (isset($_GET['userID']))
{
	...
	$aWhere[] = "`RayMusicFiles`.`Owner`=$iUser";   
}

if (isset($_GET['tag']))
{
	$sTag = htmlspecialchars_adv($_GET['tag']);
	$aWhere[] = "`RayMusicFiles`.`Tags` like '%$sTag%'";
}

if (isset($_GET['action']))
{
	$sAct = htmlspecialchars_adv($_GET['action']);
	$sAddon = defineBrowseAction($sAct,'Music',$member['ID']);
}

$sqlWhere = "WHERE " . implode( ' AND ', $aWhere ).$sAddon." AND `Approved`= 'true'";

$iTotalNum = db_value( "SELECT COUNT( * ) FROM `RayMusicFiles` $sqlWhere" );
if( !$iTotalNum )
{
...
function PageCompPageMainCode()
{
	...
	if (mysql_num_rows($rData))
	{
		while ($aData = mysql_fetch_array($rData))
		{
			...
			$sMusTitle = strlen($aData['medTitle']) > 0 ? $aData['medTitle'] : _t("_Untitled");
			...
			$sCode .= '<div class="browseUnit">';
				...
				$sCode .= '<div><a href="'.$sHref.'"><b>'.$sMusTitle.'</b></a></div>';
				$sCode .= $sProfLink;
				$sCode .= '<div>'._t("_Added").': <b>'.defineTimeInterval($aData['medDate']).'</b></div>';
				$sCode .= '<div>'._t("_Playbacks").': <b>'.$aData['medListens'].'</b></div>';
				$sCode .= $sShowRate;
				$sCode .= $sDelLink;
			$sCode .= '</div>';	
		}
