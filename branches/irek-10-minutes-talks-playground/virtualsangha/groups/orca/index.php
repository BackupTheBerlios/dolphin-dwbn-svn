...
if (!file_exists('./inc/header.inc.php')) { header ("Location: install/");  exit; } $l = 'base64_decode';
...
switch ($action)
{
	...
	case 'list_forums_admin':
		transCheck ($f->getForumsXML((int)$_GET['cat'], 1), $gConf['dir']['xsl'] . 'edit_cat_forums.xsl', $_GET['trans']);		
		break;
	...
	case 'get_new_post':
		transCheck ($f->getLivePostsXML(1, (int)$_GET['ts']), $gConf['dir']['xsl'] . 'live_tracker.xsl', $_GET['trans']);
		break;
	...
	case 'list_topics':
		transCheck ($f->getTopicsXML((int)$_GET['forum'], false, (int)$_GET['start']), $gConf['dir']['xsl'] . 'forum_topics.xsl', $_GET['trans']);
		break;

	case 'group_last_topics':
		transCheck ($f->getTopicsXML((int)$_GET['forum'], false, 0), "{$dir['root']}templates/tmpl_{$gConf['skin']}/xsl/group_last_topics.xsl", (bool)$_GET['trans']);
		break;
	
	case 'list_posts':
		transCheck ($f->getPostsXML((int)$_GET['topic'], false), $gConf['dir']['xsl'] . 'forum_posts.xsl', $_GET['trans']);
		break;
	...
	case 'new_topic':
		transCheck ($f->getNewTopicXML((int)$_GET['forum']), $gConf['dir']['xsl'] . 'new_topic.xsl', $_GET['trans']);
		break;
	...
	case 'post_success':
		transCheck ("<forum><id>{$_GET['forum']}</id></forum>", $gConf['dir']['xsl'] . 'default_post_success.xsl', $_GET['trans']);
		break;
