...
$_page['extraCodeInHead'] = <<<EOJ
	...
	<script language="javascript" type="text/javascript">
		tinyMCE.init({
			...
			extended_valid_elements : "a[name|href|title],img[class|src|border=0|alt|title|hspace|vspace|width|height|align|name],hr[class|width|size|noshade],font[face|size|color|style],span[class|align|style],script[type|language|src],div[class|align|style|id],table[class|id|style|cellspacing|cellpadding|border|bgcolor|background],tbody,tr[id|class|style],td[id|class|style],form[id|name|action|style|method],input[id|type|name|value|style|class|onclick|onchange|onfocus|onblur],select[id|name|value|style|class|onclick|onchange],textarea[id|name|value|style|class|onclick|onchange],ul[style|id|class],ol[style|id|class],li[style|id|class],label[for|style|class]"
		});
	</script>
EOJ;
...
function createNewElement( $source )
{
	...
	if( $source )
	{
		$sourceActive = (int)db_value( "SELECT `Column` FROM `$sTableName` WHERE `ID`=$source" );
		if( $sourceActive )
		{
			$sQuery = "
