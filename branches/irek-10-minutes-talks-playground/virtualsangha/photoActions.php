...
function addToFavorites($iFile)
{
	...
	if ($iFile)
	{
		$sQuery = "SELECT * FROM `sharePhotoFavorites` WHERE `medID`='$iFile' AND `userID`='{$member['ID']}'";
		...
		{
			$sQuery = "INSERT INTO `sharePhotoFavorites` (`medID`,`userID`,`favDate`) VALUES('$iFile','{$member['ID']}',NOW())";
			db_res($sQuery);
...
function sendFileInfo($iFile, $sEmail, $sMessage)
{
	...
	$sMailSubject = $aUser['NickName'].' shared a Photo with you';
	 ...
	 $sMailBody    = "Hello,\n
				{$aUser['NickName']} shared a photo with you: <a href=\"{$site['url']}viewPhoto.php?fileID=$iFile\">See it</a>\n
				$sMessage\n
				Regards";
