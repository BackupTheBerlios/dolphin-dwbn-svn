...
EOF;
	...
	function GenMemberDescrAndCat($aBlogsRes, $iCategID = -1) {
		...
		foreach( $aTagsPost as $varKey => $varValue ) {
			...
			$sTagsVals .= <<<EOF
<div style="vertical-align:middle;height:25px;margin-bottom:5px;position:relative;">
	{$sTagsImgName}
</div>
EOF;
		}
...
<div id="edited_blog_div" style="display: none; position:relative;">
	<form action="{$_SERVER['PHP_SELF']}" method="post" name="EditBlogForm">
		...
		<textarea name="Description" id="Description" rows="4" cols="10" style="width:210px;height:85px;" onkeyup="if( this.value.length > 255 ) this.value = this.value.substr( 0, 255 );">{$aBlogsRes['Description']}</textarea>
