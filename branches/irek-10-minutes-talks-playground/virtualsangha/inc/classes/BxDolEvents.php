...
EOF;
	...
	function PageSDatingShowEvents() {
		...
		$sCommonSelectSQL = "DISTINCT `SDatingEvents`.`ID`, `Title`, `Description`, `StatusMessage`, `Country`, `City`, `Place`, `PhotoFilename`, `Tags`, ";
		$sCommonSelectSQL  .= "`EventStart`, ";
...
EOF;
				...
				foreach( $aTags as $sTagKey ) {
					$sTagsHrefs .= <<<EOF
<a href="{$_SERVER['PHP_SELF']}?action=search_by_tag&amp;tagKey={$sTagKey}" >{$sTagKey}</a>&nbsp;
EOF;
				}
				$sTags = <<<EOF
...
EOF;
		...
		$sCountryPic = _t('__'. $prof['countries'][$aEventData['Country']]);
...
<a class="actions" href="{$_SERVER['PHP_SELF']}?action=search_by_tag&amp;tagKey={$varKey}" style="text-transform:capitalize;" >{$varKey}</a>&nbsp;({$varValue})
<br />
EOF;
		}
		...
		if ($aEventData['ResponsibleID']==(int)$_COOKIE['memberID']) {
			$sActions = <<<EOF
<div class="padds">
	<img src="{$site['icons']}online.gif" alt="{$sEditC}" title="{$sEditC}" class="marg_icon" />
...
EOF;
		...
		if (mysql_affected_rows() == 0) { //no matches
			$sUsersActions = <<<EOF
<div class="padds">
	<img src="{$site['icons']}online.gif" alt="{$sJoinC}" title="{$sJoinC}" class="marg_icon" />
...
EOF;
			...
			$sUsersActions = <<<EOF
<div class="padds">
	<img src="{$site['icons']}delete.png" alt="{$sUnsubscribeC}" title="{$sUnsubscribeC}" class="marg_icon" />
...
EOF;
		...
		while ( $aPartProfiles = mysql_fetch_assoc($vPartProfilesRes) ) {
			...
			$sParticipants .= DesignBoxContentBorder( process_line_output( strmaxtextlen( $aPartProfiles['NickName'], 11 ) ) . ': ' . age( $aPartProfiles['DateOfBirth'] ) . ' ' . _t('_y/o'), $sCont );
		}
...
<div class="disignBoxFirst">
	...
	<div class="boxContent">
		...
		<div class="padds" style="height:23px;vertical-align:middle;">
			<span style="vertical-align: middle;">
				<img src="{$site['icons']}cool.gif" alt="" title="" class="marg_icon" />
			</span>
			<span>
				<a class="actions" href="{$_SERVER['PHP_SELF']}?action=show_part&amp;event_id={$iEventID}">
					{$sListOfParticipantsC}
				</a>
			</span>
		</div>
...
<form id="UnsubscribeEventForm" action="{$_SERVER['PHP_SELF']}?action=show_info&amp;event_id={$iEventID}" method="post">
...
<center>
	...
	<form id="showEventsForm" action="{$_SERVER['PHP_SELF']}" method="get">
		<table cellpadding="4" cellspacing="0" border="0" class="text" width="450">
			<tr>
...
{$sJSCode}
{$sNewResText}
<form id="newEventForm" action="{$_SERVER['PHP_SELF']}{$sEventURL}" method="post" enctype="multipart/form-data">
<table class="addEventForm">
	...
	<tr class="vc">
		...
		<td class="form_value">
			...
			<textarea class="classfiedsTextArea" name="event_desc" id="event_desc_id" style="width:500px;">{$sEventDesc}</textarea>
		</td>
	</tr>
	{$sStatusMess}
	<tr class="vc">
...
EOF;
	...
	function GetEventPicture($iEventID, $sEventPicName='DOLPHIN') {
		...
		$sEventPic = <<<EOF
<div  class="thumbnail_block" style="float:left;">
	<a href="events.php?action=show_info&amp;event_id={$iEventID}">
		{$sEventPicName}
	</a>
</div>
EOF;
	...
	function GetGroupPicture($iGroupID) {
		...
		$sEventPicName = ( strlen(trim($sEventPicName)) && file_exists('groups/gallery/' . $sEventPicName) )
				? "<img class=\"icons\" style=\"width:{$this->iThumbSize}px;height:{$this->iThumbSize}px;background-image:url(groups/gallery/{$sTypePic}{$sEventPicName});\" src=\"{$sSpacerName}\" />"
				: "<img class=\"icons\" style=\"width:{$this->iThumbSize}px;height:{$this->iThumbSize}px;background-image:url({$site['url']}templates/tmpl_{$tmpl}/{$this->sPicNotAvail});\" src=\"{$sSpacerName}\" />";
		$sEventPic = <<<EOF
<div  class="thumbnail_block" style="float:left;">
