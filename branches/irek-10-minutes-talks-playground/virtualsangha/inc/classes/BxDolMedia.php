...
class BxDolMedia extends BxDolMistake
{
	...
	function getMediaConfigArray()
	{
		...
		global $max_video_files;
		global $max_video_size;
		global $max_audio_files;
		global $max_audio_size;
		...
		$aMediaConfig['enable']['audio'] = $enable_audio_upload;
		$aMediaConfig['enable']['video'] = $enable_video_upload;
		...
		switch( $this -> sMediaType )
		{
			...
			break;
			case 'audio':
				$aMediaConfig['dir']['audio'] = $dir['profileSound'];
				$aMediaConfig['url']['audio'] = $site['profileSound'];
				$aMediaConfig['max']['audio'] = (int)$max_audio_files ? (int)$max_audio_files : 20; // Max number of audio files
				$aMediaConfig['max']['audioFile'] = $max_audio_size * 1024 * 1024; // in MB
			break;
			case 'video':
				$aMediaConfig['dir']['video'] = $dir['profileVideo'];
				$aMediaConfig['url']['video'] = $site['profileVido'];
				$aMediaConfig['max']['video'] = (int)$max_video_files ? (int)$max_video_files : 20; // Max number of video files
				$aMediaConfig['max']['videoFile'] = $max_video_size * 1024 * 1024; // in MB
			break;
		}
