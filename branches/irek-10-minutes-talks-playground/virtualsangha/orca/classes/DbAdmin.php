...
class DbAdmin extends BxDb
{
	...
	function getCatIdByForumId ($forum_id)
	{
		$sql = "SELECT `cat_id` FROM " . TF_FORUM . " WHERE `forum_id` = '$forum_id' LIMIT 1";		
	...
	{
		$sql = "INSERT INTO " . TF_FORUM_CAT . " SET `cat_name` = '$cat_name', `cat_order` = " . ($this->getCatOrder (0) + CAT_ORDER_STEP);
		return $this->query ($sql);
	}		
	...
	function insertForum ($cat_id, $title, $desc, $type)
    {		        
		$sql = "INSERT INTO " . TF_FORUM . " SET `cat_id` = '$cat_id', `forum_title` = '$title', `forum_desc` = '$desc', `forum_type` = '$type'";
		return $this->query ($sql);
	}
