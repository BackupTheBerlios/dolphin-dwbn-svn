...
$sQuery = "SELECT `ID`, 
				  ...
				  `Email`,
				  `Sex`,
				   DATE_FORMAT(`LastLoggedIn`,  '$date_format' ) AS `LastLoggedInCur`,
				   DATE_FORMAT(`LastReg`,  '$date_format' ) AS `LastReg`,
				   `Status`
...
?>
	<table cellspacing=0 cellpadding=0 border="0" align="right" class="text"><tr><td><a href="<? echo $site[url]; ?>profile_edit.php">Add a new profile</a></td></tr></table><br>
...
<tr class=<? echo $col; ?> bgcolor="#ffffff">
	...
	<td>&nbsp;<a href="../profile_edit.php?ID=<? echo $p_arr[ID]; ?>"><? echo $p_arr[ID]; ?></a>&nbsp;</td>
	
	<td>&nbsp;<? echo $p_arr['NickName']; ?>&nbsp;</td>
	
	<td>&nbsp;<? echo $sEmail; ?>&nbsp;</td>
	
	<td align=center><?echo $p_arr['LastReg']?></td>
	
	<td align=center><? echo $p_arr['LastLoggedInCur']; ?></td>
	
	<td><?echo getUserMedia($p_arr['ID'], 'photo');?></td>
	
	<td align="center"><?echo getUserMedia($p_arr['ID'], 'audio');?></td>
	
	<td align="center""><?echo getUserMedia($p_arr['ID'], 'bg');?></td>
