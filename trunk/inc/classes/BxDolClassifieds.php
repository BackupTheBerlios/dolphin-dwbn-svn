	...
	function ActionBuySendMailAdvertisement($iAdvertisementID) {
		...
		if ($iMemberID > 0 /*AND $iMemberID != $nameRet['ID']*/ ) {
			...
			if ($aSqlResStr) {
				...
				$aPlus['Email'] = $aSqlSellerRes['Email'];
				...
				if (sendMail( $aSqlMemberRes['Email'], $sSubject, $sMessageB, $aSqlSellerRes['ID'], $aPlus, $aSqlMemberRes['NickName'], $aSqlMemberRes['Email'])) {
				...
				if (sendMail( $aSqlSellerRes['Email'], $sSubjectS, $sMessageS, $aSqlSellerRes['ID'], $aPlus, $aSqlMemberRes['NickName'], $aSqlMemberRes['Email'])) {
					$sRetHtml .= MsgBox(_t('_Email was successfully sent'));
				}
...
EOF;
	...
	function PrintFilteredAllAdvertisements() {
		...
		$sCustom1Cat = ($sCustAction1 == '' OR $sCustomFieldValue1 == '')?'':"CAST(`ClassifiedsAdvertisements`.`CustomFieldValue1`AS UNSIGNED) {$sCustAction1} {$sCustomFieldValue1}";
		$sCustom2Cat = ($sCustAction2 == '' OR $sCustomFieldValue2 == '')?'':"CAST(`ClassifiedsAdvertisements`.`CustomFieldValue2`AS UNSIGNED) {$sCustAction2} {$sCustomFieldValue2}";
