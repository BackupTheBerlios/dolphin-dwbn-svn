...
//widget name
$sWidget = isset($_REQUEST['widget']) ? $_REQUEST['widget'] : "";
//widget status
$bWidgetEnable = isset($_REQUEST['enable']) ? $_REQUEST['enable'] == 'true' : true;
//folder name
$sFolderName = isset($_REQUEST['folder']) ? $_REQUEST['folder'] : '';
//default file
$sDefaultFile = isset($_REQUEST['default']) ? $_REQUEST['default'] : '';
//file
$sFile = isset($_REQUEST['file']) ? $_REQUEST['file'] : '';

//setting key
$sSettingKey = isset($_REQUEST['key']) ? $_REQUEST['key'] : "";
//setting value
$sSettingValue = isset($_REQUEST['value']) ? $_REQUEST['value'] : "";
...
switch ($sAction)
{
	...
	/**
        ...
        case 'finishInstall':
                ...
                if(!file_exists($sWidgetFileName))
                {
                        $sContents = parseXml($aXmlTemplates['result'], getError($aErrorCodes[1], $sWidgetFile), FAILED_VAL);
                        break;
                }
                ...
                if(empty($sResult))
                        ...
                        $sContents = parseXml($aXmlTemplates['result'], $sResult, FAILED_VAL);
        ...
        case 'checkPermissions':
                ...
                if(empty($sResult))
                        ...
                        $sContents = parseXml($aXmlTemplates['result'], $sResult, SUCCESS_VAL);
                break;
        ...
        case 'getMyWidgets':
                ...
                while(($sInner = readdir($rDirHandler)) !== false)
                        if(is_dir($sModulesPath . $sInner) && substr($sInner, 0, 1) != '.' && $sInner != 'global')
                        {
                                ...
                                $sAdminUrl = file_exists($sModulesPath . $sInner . "/app/admin.swf") ? $sModulesUrl . $sInner . "/app/admin.swf" : "";
