...
switch ($sAction)
{
	...
	*/
	case 'userAuthorize':
		...
		{
			$aAdmin = getArray("SELECT * FROM `Admins` LIMIT 1");
			$aProfile = getArray("SELECT * FROM `" . MODULE_DB_PREFIX . "Profiles` WHERE `ID`='" . $sId . "'");
			if(!is_array($aProfile) || count($aProfile) == 0)
			{
				$aProfile = array("Type" => CHAT_TYPE_ADMIN);
				getResult("INSERT INTO `" . MODULE_DB_PREFIX . "Profiles` SET `ID`='" . $sId . "', `Type`='" . $aProfile["Type"] . "', `Smileset`='" . $sDefSmileset . "'");
			}
			$iCurrentTime = time();
			getResult("REPLACE `" . MODULE_DB_PREFIX . "CurrentUsers` SET `ID`='" . $sId . "', `Nick`='" . $aAdmin['Name'] . "', `Sex`='M', `Age`='35', `Desc`='Administrator', `Photo`='" . $sManImageUrl . "', `Profile`='', `Start`='" . $iCurrentTime . "', `When`='" . $iCurrentTime . "', `Status`='" . USER_STATUS_NEW . "'");
			...
			$sContents .= parseXml($aXmlTemplates['user'], $sId, USER_STATUS_NEW, $aAdmin['Name'], "M", "30", "Administrator", $sManImageUrl, "", $aProfile['Type'], USER_STATUS_ONLINE);
		}
		elseif(loginUser($sId, $sPassword) == TRUE_VAL && ($bBanned = doBan("check", $sId)) != TRUE)
		{
			...
			if(!is_array($aProfile) || count($aProfile) == 0)
			{
				$aProfile = array("Type" => CHAT_TYPE_FULL);
				getResult("INSERT INTO `" . MODULE_DB_PREFIX . "Profiles` SET `ID`='" . $sId . "', `Type`='" . $aProfile["Type"] . "', `Smileset`='" . $sDefSmileset . "'");
			}
			$iCurrentTime = time();
			...
			$sContents = parseXml($aXmlTemplates['result'], "", SUCCESS_VAL);
			$sContents .= parseXml($aXmlTemplates['user'], $sId, USER_STATUS_NEW, $aUser['nick'], $aUser['sex'], $aUser['age'], $aUser['desc'], $aUser['photo'], $aUser['profile'], $aProfile['Type'], USER_STATUS_ONLINE);
		}
		else $sContents = parseXml($aXmlTemplates['result'], $bBanned ? "msgBanned" : "msgUserAuthenticationFailure", FAILED_VAL);
		break;
