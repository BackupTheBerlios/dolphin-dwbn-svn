...
function BxDolPFM( aAreas ) {
	this.config = {
		...
		parserUrl: '',
		inactiveColumns: 4,
		getAreaElem : function(ind) { //function to get the area element. made to customizability
			return $( '#m' + ind + ' > div.build_container' ).get(0);
		}
	};
...
BxDolPFM.prototype.init = function() {
	//generate areas
	while( this.areas.length < this.config.areas ) {
		this.areas[this.areas.length] = new BxDolPFMArea(this, ( this.areas.length + 1 ) ); //we will begin ID's from 1 !
	}
}
...
BxDolPFMArea.prototype.draw = function() {
	...
	$( this.element ).append(
		'<div class="build_zone_header">Active Items</div>' +
		...
		'<div class="build_zone_header">Inactive Blocks</div>' +
		'<div class="blocks_cont_bord">' +
		...
		'</div>' +
		'<br style="height:20px;clear:both;" />' +
		'<div class="build_zone_header">Inactive Items</div>' +
...
BxDolPFMArea.prototype.activateSortable = function() {
	...
	this.sortableInstanceBlocks = $( this.element ).sortable({
		items: 'div.build_block,div.build_block_fake',
		hoverClass: 'build_block_hover',
		stop: function(a,b){
			oThisArea.stopItemsSort( this.id );
		}
	}).sortableInstance(); //create instance for destroying possibilities

	this.sortableInstanceItems = $( this.element ).sortable({
		items: 'div.build_item_active,div.build_item_inactive,div.build_item_fake',
		hoverClass: 'build_item_hover',
		stop: function(a,b){
			oThisArea.stopItemsSort( this.id );
		}
	}).sortableInstance();
...
BxDolPFMArea.prototype.deactivateSortable = function() {
	this.sortableInstanceBlocks.destroy();
	this.sortableInstanceItems.destroy();
}

BxDolPFMArea.prototype.stopItemsSort = function( draggedElementID ) {
...
BxDolPFMArea.prototype.openFieldDialog = function( iItemID, iAreaID ) {
	...
	$( '#edit_form_cont' ).html( '<div class="notify_msg">Loading...</div>' ).load( this.parent.config.parserUrl + '?action=loadEditForm&id=' + iItemID + '&area=' + iAreaID );
}
...
function hideEditForm() {
	//if( confirm( 'Close properties window?' ) )
		$( '#fieldFormWrap' ).css({display:'none'});
}
...
function genEditFormError( sField, sText ) {
	if( document.forms.fieldEditForm[sField] ) {
		$( document.forms.fieldEditForm[sField] ).parent( 'td' ).addClass( 'warned' ).next( 'td' ).prepend(
			'<img src="images/depr.gif" class="depr_icon" ' + 
