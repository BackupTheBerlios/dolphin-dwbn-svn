...
EOF;
			...
			case 'user_perm':				
                ...
                $isLoggedIn = $logged['admin'] || ($memberID && $user) ? 1 : 0;

                $aCheckRes = checkAction( $memberID, ACTION_ID_USE_ORCA_PUBLIC_FORUM, true );
                $isPublicForumAllowed = CHECK_ACTION_RESULT_ALLOWED == $aCheckRes[CHECK_ACTION_RESULT] && $isLoggedIn ? 1 : 0;

                $aCheckRes = checkAction( $memberID, ACTION_ID_USE_ORCA_PRIVATE_FORUM, true );
                $isPrivateForumAllowed = CHECK_ACTION_RESULT_ALLOWED == $aCheckRes[CHECK_ACTION_RESULT] && $isLoggedIn ? 1 : 0;
				...
				$search = $memberID || $isOrcaAdmin ? 1 : 0;
