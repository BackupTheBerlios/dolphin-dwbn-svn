function BxDolMenu( idContainer, urlParser, aTopItems, aCustomItems, aSystemItems, aInactiveItems, aCoords, e )
{
	...
	{
		var oNewSItem = new BxDolMenuItem( this, id, 'system', aSystemItems[id], this.iCounterX, 0 );
		...
		{
			var oNewCItem = new BxDolMenuItem( this, cid, 'custom', aCustomItems[id][cid], this.iCounterX, ( iCounterY + 1 ) );
	...
	{
		var oNewItem = new BxDolMenuItem( this, id, 'top', aTopItems[id], this.iCounterX, 0 );
...
BxDolMenu.prototype.checkCollisions = function()
{
	...
	else if( obj.type == 'top'    && obj.dragToCellY > 0  && obj.dragToCellX >= 0 && obj.dragToCellX < ( this.iCounterX - 1 ) && obj.dragToCellY <= ( /*this.aCounterY[obj.dragToCellX] + */ 1 ) )
	{
	...
	else if( obj.type == 'inactive' && obj.dragToCellY > 0  && obj.dragToCellX >= 0 && obj.dragToCellX < this.iCounterX && obj.dragToCellY < ( this.aCounterY[obj.dragToCellX] + 2 ) )
	{
...
BxDolMenu.prototype.resortItemsArrs = function()
{
	...
	for( ind = 0; ind < iItemsNum; ind ++ )
	{
		...
		else if( obj.cellY == 0 && obj.type == 'inactive' )
		{
			...
			if( !newObjID )
			{
				alert( 'Sorry couldn\'t insert object' );
				continue;
			}
...
BxDolMenu.prototype.resizePseudos = function()
{
	...
	this.pseudo1.style.height = ( ( this.iMaxCounterY + 2 ) * this.iDiffY ) + 'px';

	this.pseudo2.innerHTML = '';
	this.pseudo2.style.width  = ( ( this.iInactivePerRow     ) * this.iDiffX ) + 'px';
	this.pseudo2.style.height = ( ( this.iInactiveRowCounter ) * this.iDiffY ) + 'px';
...
function BxDolMenuItem( parentObj, id, type, title, cellX, cellY, sourceID )
{
...
function itemClickEvent( e, obj )
{
	if( obj.type == 'inactive' || obj.type == 'system' )
		alert( 'This items are non-editable' );
	else
	{
