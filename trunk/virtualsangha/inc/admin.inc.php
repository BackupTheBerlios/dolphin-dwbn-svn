...
function login_form( $text = "", $member = 0, $bbAjaxMode = false )
{
	...
	$sPath = dirname( $_SERVER['PHP_SELF'] ); //we got [/some/path/to]
	$sCDir = basename( $sPath ); // get [to].
...
function PageCompLoginForm( $text, $member, $mem, $table, $login_page, $join_page, $forgot_page, $template = '' )
{
	...
	$aFormReplace['form_onsubmit']  = "validateLoginForm( this, '{$site['url']}', '" .
	  addslashes( htmlspecialchars( _t('_PROFILE_ERR') ) ) .
	  "'); return false;";
...
?>
				<tr>
					<td><?= _t("_Results") ?>:&nbsp;<b><?= (int)$page_first_p ?></b>-<b><?= min((int)($page_first_p + $p_per_page - 1), (int)$p_num) ?></b>&nbsp;|&nbsp;<?= _t("_Total") ?>:&nbsp;<b><?= (int)$p_num ?></b></td>
